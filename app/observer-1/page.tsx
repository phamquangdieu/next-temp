...
const Obeserver1 = () => {
  ...
  useGSAP(
    () => {
      ...
      const rootImages: HTMLImageElement[] = gsap.utils.toArray(
        `.${styles.image}`
      );
      const images = rootImages.reverse();
      console.log(sections, images);

      const slideImages: HTMLImageElement[] = gsap.utils.toArray(
        `.${styles.slide_img}`
      );
      const outerWrappers: HTMLDivElement[] = gsap.utils.toArray(
        `.${styles.slide_outer}`
      );
      const innerWrappers: HTMLDivElement[] = gsap.utils.toArray(
        `.${styles.slide_inner}`
      );
    ...
    <div ref={ref} className={styles.container}>
      ...
      <section className={styles.overlay}>
        <div className={styles.overlay_content}>
          <p className={styles.overlay_count}>
            0<span className={styles.count}>1</span>
          </p>
