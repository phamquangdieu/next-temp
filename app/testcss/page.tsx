...
const TestCSSPage = () => {
    ...
    useEffect(() => {
        ...
        const customMarked = (markdown: string, options?: Parameters<typeof marked>[1]) => {
    ...
    const editor = useEditor({
        ...
        content: '<p>Hello World! 🌎️</p><h1>This is H1</h1><h2>This is H2</h2><h3>This is H3</h3><p>Try <strong>bold</strong> and <em>italic</em> text</p>',
        onUpdate: ({ editor }) => {
    ...
    return (
        <div className="p-4">
            <div className="mb-4">
                <Button onClick={() => editor?.chain().focus().toggleBold().run()} className="mr-2">Bold</Button>
                <Button onClick={() => editor?.chain().focus().toggleItalic().run()} className="mr-2">Italic</Button>
                <Button onClick={() => editor?.chain().focus().toggleHeading({ level: 1 }).run()} className="mr-2">H1</Button>
                <Button onClick={() => editor?.chain().focus().toggleHeading({ level: 2 }).run()} className="mr-2">H2</Button>
                <Button onClick={() => editor?.chain().focus().toggleHeading({ level: 3 }).run()} className="mr-2">H3</Button>
                <Button onClick={() => setMarkdownContent(getMarkdownContent())} className="mr-2">Get Markdown</Button>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                    <h3 className="mb-2 font-medium">Rendered Markdown (Custom Syntax):</h3>
                    <div 
                        className="border p-4 rounded prose"
                        dangerouslySetInnerHTML={{ __html: parseMarkdown(getCustomSyntaxMarkdown()) }}
                    />
                </div>
            </div>
        </div>
